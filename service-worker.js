const CACHE_NAME = 'resto-calculator-v2'; // <-- ТОВА Е КРИТИЧНАТА ПРОМЯНА
// Файловете, които искаме да кешираме:
const urlsToCache = [
  '/',
  'index.html',
  'manifest.json',
  'service-worker.js', // Кешираме и самия себе си
  'icon-192.png',
  'icon-512.png'
];
...
self.addEventListener('fetch', event => {
  event.respondWith(
    caches.match(event.request)
      .then(response => {
        // Ако има кеширан отговор, го връщаме.
        if (response) {
          return response;
        }
        // В противен случай, опитваме да изтеглим от мрежата.
        return fetch(event.request).catch(function() {
            // Ако няма мрежа и файлът не е в кеша, връщаме index.html като резервен вариант
